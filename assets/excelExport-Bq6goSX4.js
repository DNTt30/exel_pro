import{N as e,T as t,U as n,b as r,ct as i,k as a,n as o,st as s}from"./index-DiyZy1P5.js";import{t as c}from"./createLucideIcon-BYkeGIvI.js";import{t as l}from"./shallow-B8IfzWG1.js";var u=c(`printer`,[[`path`,{d:`M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2`,key:`143wyd`}],[`path`,{d:`M6 9V3a1 1 0 0 1 1-1h10a1 1 0 0 1 1 1v6`,key:`1itne7`}],[`rect`,{x:`6`,y:`14`,width:`12`,height:`8`,rx:`1`,key:`1ue0tg`}]]),d=i(s(),1);function f(e,i,a,s=null){let{employees:c,user:u}=o(l(e=>({employees:e.employees,user:e.user}))),f=r(u);return(0,d.useMemo)(()=>{let r=c;if(e){let t=e.toLowerCase();r=r.filter(e=>e.name.toLowerCase().includes(t)||e.id.toLowerCase().includes(t))}a!==`ALL`&&(r=r.filter(e=>(e.role||e.type)===a));let o=f?i:u?.dept,l={};if(r.forEach(e=>{if(l[e.dept]||(l[e.dept]=[]),l[e.dept].push({...e,isBorrowedTo:null}),s){let r=s[e.id]||{},i=new Set;n.forEach(n=>{let a=r[n],o=t(a);o&&o!==e.dept&&i.add(o)}),i.forEach(t=>{l[t]||(l[t]=[]),l[t].push({...e,isBorrowedTo:t})})}}),o&&o!==`ALL`){let e={};return l[o]&&(e[o]=l[o]),e}return l},[c,e,i,a,f,u?.dept,s])}function p({currentWeek:t,deptName:n,groupedEmps:r,weekSchedule:i}){let o=t.split(`-`),s=parseInt(o[0],10),c=parseInt(o[1],10),l=parseInt(o[2],10),u=[`T2`,`T3`,`T4`,`T5`,`T6`,`T7`,`CN`],d=new Date(s,c-1,l),f=u.map((e,t)=>{let n=new Date(d);n.setDate(d.getDate()+t);let r=n.getDate().toString().padStart(2,`0`),i=(n.getMonth()+1).toString().padStart(2,`0`);return{dayKey:e,dateStr:`${r}/${i}`,header:`${e} (${r}/${i})`}}),p=new Date(d);p.setDate(d.getDate()+6);let m=`${`${d.getDate().toString().padStart(2,`0`)}/${(d.getMonth()+1).toString().padStart(2,`0`)}`} → ${`${p.getDate().toString().padStart(2,`0`)}/${(p.getMonth()+1).toString().padStart(2,`0`)}`}/${s}`,h=e=>{if(!e)return`color: #94a3b8;`;let{shift:t,covering_store:n}=a(e);return!t||t===`off`?`color: #94a3b8;`:n?`background-color: #fef08a; color: #854d0e; font-weight: bold;`:t===`6-14`?`background-color: #bbf7d0; color: #166534; font-weight: bold;`:t===`14-22`?`background-color: #bfdbfe; color: #1e40af; font-weight: bold;`:t===`10-18`?`background-color: #fed7aa; color: #9a3412; font-weight: bold;`:t===`22-6`?`background-color: #fecaca; color: #991b1b; font-weight: bold;`:`background-color: #e2e8f0; color: #334155; font-weight: bold;`},g=``,_=1;Object.entries(r).forEach(([t,n])=>{g+=`
      <tr style="background-color: #e0e7ff; font-weight: bold; color: #1e3a8a;">
        <td colspan="${f.length+6}" style="text-align: left; padding: 8px 12px; font-size: 11pt; border: 1px solid #94a3b8;">
          🏬 CỬA HÀNG: ${t} (${n.length} nhân sự)
        </td>
      </tr>
    `,n.forEach(n=>{let r=i[n.id]||{},o=0,s=0,c=f.map(t=>{let n=r[t.dayKey]||``,{shift:i,covering_store:c}=a(n);if(i&&i!==`off`){if(s++,e[i])o+=e[i].hours;else{let e=i.match(/^(\d+)[hH]?(?:\s*-\s*|\s+)(\d+)[hH]?$/);if(e){let t=parseInt(e[1],10),n=parseInt(e[2],10);n<t&&(n+=24),o+=n-t}}}return`<td style="border: 1px solid #94a3b8; text-align: center; mso-number-format: '\\@'; ${h(n)}">${c?`${i} ${c}`:i||`-`}</td>`}).join(``),l=(n.type===`PARTTIME`||n.type===`STPT`||n.role&&n.role.includes(`PT`))&&o>23,u=l?`background-color: #fee2e2; color: #b91c1c; font-weight: bold;`:`background-color: #f8fafc; color: #1e40af; font-weight: bold;`;g+=`
        <tr>
          ...
          <td style="border: 1px solid #94a3b8; text-align: center; font-weight: bold; color: #2563eb;">${n.dept||t}</td>
          ${c}
