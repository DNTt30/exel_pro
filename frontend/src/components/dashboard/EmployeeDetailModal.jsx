...
// Modal chi tiet nhan vien â€” tach tu Dashboard.jsx.
export default function EmployeeDetailModal({ emp, onClose, viewMode = 'week', selectedMonthCycle = '', cycleDates = [] }) {
  if (!emp) return null;
  return (
              ...
              <div>
                ...
                {viewMode === 'month' ? (
                  <div className="grid grid-cols-7 gap-1.5 text-center bg-slate-50 p-2.5 rounded-xl border border-slate-200">
                    {cycleDates.map(d => {
                      const val = emp.monthShifts[d.key];
                      return (
                  ...
                  <div className="grid grid-cols-7 gap-2 text-center bg-slate-50 p-3 rounded-xl border border-slate-200">
                    {WEEK_DAYS.map(day => {
                      const shiftVal = emp.weekShifts[day];
                      return (
